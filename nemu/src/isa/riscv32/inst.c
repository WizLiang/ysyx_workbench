...
//jal ret mv j 
  ...
  INSTPAT("??????? ????? ????? 110 ????? 00100 11",  ori   , I, R(rd) = src1 | (int32_t)imm );
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(11, s->pc)); //ecall 在用户模式下是 cause = 8，超级模式 cause = 9，机器模式 cause = 11 等等
